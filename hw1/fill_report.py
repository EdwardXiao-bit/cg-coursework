...
# 小组成员（本作业为 2 人小组）。模板里只有一组“姓名/学号”占位，生成后需要
# 手工再补一行；当前入库的 hw1/report/HW1-报告.docx 首页为：
#     姓名 肖铭皓   学号 2024141460455
#     姓名 张原野   学号 2024141460416
MEMBERS = [("肖铭皓", "2024141460455"), ("张原野", "2024141460416")]

# 注意：模板 HW1_todo\HW1-报告模板.docx 不在本仓库中，需上级目录存在 HW1_todo\
# 并放好模板后才能重新运行本脚本。已生成的 hw1/report/HW1-报告.docx 已入库，
# 正文若再修改，请保持本文件与 docx 两边一致。
...
THOUGHT = [
    ...
    "基本要求的实现分三步。第一步是几何数据准备：我们把点、线、面所需的基本图元顶点统一放在全局数组 points 中，"
    ...
    "我们选用 Sierpinski 三角垫片，它的生成规则很简洁：对任意三角形，取三边中点并互相连接，"
    ...
    "交互上，我们在页面左上角放置了一个 range 类型的 slider（范围 0~7），监听它的 input 事件："
...
DIFFICULTY = [
    ...
    "我们一开始只做了线性映射而忘了翻转，结果点击画布上方图形却跑到下方。"
    ...
    "我们一开始只改了 depth 就直接重绘，画面毫无变化，因为 GPU 里还是旧的那批顶点。"
    "后来才明白 VBO 不会自己跟着 CPU 端的数组走，必须重新 bindBuffer + bufferData 把新数据传上去。"
    ...
    "但每次滑动都要在 CPU 端重新递归生成全部顶点，所以我们把上限设为 7，"
    "在视觉效果与交互流畅度之间取了一个平衡。",
]

INSIGHT = [
    "通过这次作业，我们最主要的收获是把 WebGL 的绘制流程真正串了起来："
...
def main():
    ...
    #    模板里只有一组占位，2 人小组需再复制一行填写第二位成员（见上面的 MEMBERS）
