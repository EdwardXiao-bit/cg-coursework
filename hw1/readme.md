...
- **图形平移**：我们不重新计算顶点数据，而是在顶点着色器中统一加上中心偏移量：
...
- **递归细分**：我们对三角形取三边中点，连接后得到 3 个与原三角形相似、面积为 1/4 的小三角形，递归处理这 3 个小三角形；
...
1. **直接用 `file://` 打开报 `Could not find shader source`**
   —— `XMLHttpRequest` 读取本地文件被同源策略拦截，我们改用本地 HTTP 服务器访问后解决。

2. **滑块拖不动、看不到控件**
   —— canvas 是 `position: fixed` 且铺满全屏，会吞掉上层鼠标事件。
   我们给控制面板设置 `position: fixed; z-index: 10`，使其稳定位于 canvas 之上后解决。
   另外若浏览器缓存了旧版 `hw1.html`，页面不会有控件，需强制刷新（`Ctrl + F5`）。

3. **着色器中 uniform 变量名写错**
   —— TODO3 注释里写的是 `colorRandom`，而片元着色器实际声明的是 `randomColor`，
   `getUniformLocation` 取不到位置且不会报错，颜色不会改变。我们最终统一以着色器中的声明为准。
...
- `Common/` 下的 `webgl-utils.js`、`initShaders2.js`、`MVnew.js` 为课程提供的公共库，我们未作任何修改。
- 全部功能仅依赖浏览器原生 WebGL 2.0，无需安装依赖即可运行。

## 八、提交包目录结构

`hw1/submission/HW1.zip` 解压后得到一层 `HW1/` 目录，结构如下：
...
提交包默认**不含**演示视频：`2024HW1基本功能完成效果.mp4` 仍保留在 `hw1/report/` 下，
需要一并打包时加 `-IncludeVideo`。

在仓库根目录执行下面的命令即可重新生成提交包：

```powershell
.\tools\build-submission.ps1 -Hw hw1                  # 生成 hw1\submission\HW1.zip（不含视频）
.\tools\build-submission.ps1 -Hw hw1 -IncludeVideo    # 附带演示视频
```

> 运行提示：`executable/` 下双击 `start-server.bat` 即可（自动启动本地 HTTP 服务器并打开浏览器）。
> 直接双击 `hw1Code/hw1.html` 会因浏览器同源策略拦截 AJAX 读取着色器而无法运行，这是本作业唯一的运行注意事项。
